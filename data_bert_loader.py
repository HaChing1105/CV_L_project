...
class Vqa_bert_Dataset(data.Dataset):
    # max_num_ans=10
    def __init__(self, input_dir, input_vqa, max_qst_length=30, max_num_ans=10, transform=None):
        ...
        self.vqa = np.load(input_dir + '/' + input_vqa, allow_pickle=True)  # allow_pickle=True to aviod value error
    ...
    def __getitem__(self, idx):
        ...
        bert_sample = {'image': image, 'question': bert_qst2idc}
        # sample = {'image': image, 'question': bert_qst2idc}

        if load_ans:
            # bert
            ans2idc = [ans_vocab.bert_token2id(w) for w in vqa[idx]['valid_answers']]
            ...
            bert_sample['answer_label'] = ans2idx
            ...
            bert_sample['answer_multi_choice'] = mul2idc

        if transform:
            # sample['image'] = transform(sample['image'])
            bert_sample['image'] = transform(bert_sample['image'])
...
def get_bert_loader(input_dir, input_vqa_train, input_vqa_valid, max_qst_length, max_num_ans, batch_size, num_workers):
    ...
    vqa_dataset = {
        'train': Vqa_bert_Dataset(
            input_dir=input_dir,
            input_vqa=input_vqa_train,
            max_qst_length=max_qst_length,
            max_num_ans=max_num_ans,
            transform=transform['train']),
        'valid': Vqa_bert_Dataset(
            input_dir=input_dir,
